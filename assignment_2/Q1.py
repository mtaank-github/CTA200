...
for i in range(len(c_vals)):
    ...
    if modulus_of_c[i] > 1.0:
        plt.plot(reals[i], imags[i], 'r-', label="c = "+str(c_vals[i]))
    else:
        plt.plot(reals[i], imags[i], 'b-', label="c = "+str(c_vals[i]))

plot_setup()


#For the plot with colour scale
for i in range(len(c_vals)):
    x = np.linspace(1,n,n)
    c = x
    
    plt.scatter(reals[i], imags[i], c=c)
    plt.plot(reals[i], imags[i], '-', label="c = "+str(c_vals[i]))

#We want the colour change to represent the current iterations
